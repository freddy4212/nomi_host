  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar max-h-[360px]">
      <div v-if="loading" class="flex justify-center py-8">
        <div class="animate-spin rounded-full h-6 w-6 border-b-2 border-nomi-blue"></div>
      </div>
    ...
    <div class="mt-4 pt-4 border-t border-white/10">
      <button 
        @click="addDevice"
        class="w-full py-2.5 bg-nomi-blue hover:bg-nomi-blue/90 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors shadow-lg shadow-nomi-blue/20"
      >
        <Plus class="w-4 h-4" />
        新增裝置
      </button>
